...
MODEL_SAVE_PATH = os.path.join(DATA_DIR, "optimized_Grid_RBF_SVM_model_final.pkl")
SAFE_MEMORY_LIMIT = 11 * 1024**3

# 路径配置
LOG_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "Grid_logs")
...
SVM_GRID_PARAMS = {  # 修改这里的参数组合数量
    'C': [0.1, 1, 10],          # ← 减少元素数量可减少迭代次数
    'gamma': [1e-4, 1e-2, 1e-1, 1],  # ← 减少元素数量可减少迭代次数
    'class_weight': [None, 'balanced'],  # ← 可选参数
    'decision_function_shape': ['ovo', 'ovr']  # ← 可选参数
}

# SVM_GRID_PARAMS = {
#     'C': [0.1],          # 从3个减到2个
#     'gamma': [1e-3],  # 从4个减到2个
#     'class_weight': ['balanced'],  # 从2个减到1个
#     # 移除decision_function_shape参数
# }
...
class EnhancedEvaluator:
    @staticmethod
    def log_parameter_update(iteration, params, scores, logger, memory_guard):
        """记录带资源监控的单个参数组合评估结果"""
        stats = memory_guard.get_cpu_stats()
        log_entry = (
            f"迭代{iteration}/48 | "
            f"分数: {np.mean(scores):.4f} | "
    ...
    @staticmethod
    def save_best_params(optimizer, logger):
        """保存完整训练日志"""
        ...
        log_content += f"CPU使用峰值: {process.cpu_percent()}%\n\n"
...
def main():
    ...
    try:
        ...
        for i in range(len(grid_search.cv_results_['params'])):
            ...
            EnhancedEvaluator.log_parameter_update(
                i, 
                params, 
                [mean_score],
                logger,
        ...
        # 执行增强可视化分析
        monitor_data = {
            'iterations': list(range(len(grid_search.cv_results_['mean_fit_time']))),
            'scores': grid_search.cv_results_['mean_test_score'],
            'memory': [psutil.virtual_memory().percent for _ in grid_search.cv_results_['params']],
            'cpu': [psutil.cpu_percent() for _ in grid_search.cv_results_['params']]
        }
        ...
        metrics = EnhancedEvaluator.evaluate(
            ...
            grid_search.cv_results_['mean_fit_time'][-1],
            infer_time
        )
        ...
        logger.info(
            "\n=== 资源使用总结 ===\n"
            f"内存峰值: {mem_guard.peak_mem/1024**3:.2f}GB\n"
...
def setup_logger():
    ...
    if not logger.handlers:
        ...
        file_handler = logging.FileHandler(
            os.path.join(LOG_DIR, f"Grid_SVM_training_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
        )
...
if __name__ == "__main__":
    main()
from visualization import simhei_font